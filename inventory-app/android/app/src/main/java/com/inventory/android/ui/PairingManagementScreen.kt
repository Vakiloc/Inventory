...
fun PairingManagementScreen(
  ...
  Column(
    ...
    Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
      ...
      Button(onClick = { onDone() }) { Text(t("pairing.button.done")) }
      Spacer(Modifier.weight(1f))
      TextButton(onClick = {
        scope.launch {
          prefs.clearAll()
          SyncScheduler.cancelAll(context)
          status.value = I18n.t(context, "pairing.status.unpaired")
          onDone()
        }
      }) { Text(t("pairing.button.unpair")) }
    }
        ...
        Row(
          ...
          Button(onClick = {
            scope.launch {
              ...
              // Append local-generated inventory into the selected desktop inventory.

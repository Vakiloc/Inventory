// We use regex for local IP checking to avoid dependencies
...
export function requireLocalNetwork(req, res, next) {
  ...
  // If trusting proxies (e.g. running behind Nginx), use req.ip or x-forwarded-for
  // But for this local-first app, we usually look at the direct socket to avoid spoofing 
  // unless explicitly configured to trust a proxy.
  // const ipToCheck = req.ip || remoteAddress; 

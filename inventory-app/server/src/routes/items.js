...
export function createItemsRouter({ requireAuth, requireEdit }) {
  ...
  // Incremental sync helper for mobile: fetch barcode mappings created after a given time.
  router.get(
    '/item-barcodes',
    requireAuth,
    wrapRoute((req, res) => {
  ...
  // Composite sync endpoint: push changes then pull changes since timestamp
  router.post(
    '/sync',
    requireAuth,
    requireEdit,

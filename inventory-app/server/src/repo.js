...
export function forceAttachBarcodeToItem(db, itemId, barcode) {
  ...
  const tx = db.transaction(() => {
    ...
    // If barcode already set as primary on this item, treat as success
...
export function getItemsByBarcodeExact(db, barcode) {
  ...
  // Prefer explicit alternate-barcode mappings (1 barcode -> 1 item) over primary barcode matches.
  // This allows clients to "override" a duplicated primary barcode by attaching it as an alternate.
  if (viaAlt.length > 0) {
    const seen = new Set();
    const out = [];
...
export function upsertManyByIdLww(db, incomingItems) {
  const tx = db.transaction(() => {
    for (const item of incomingItems) {
      ...
      if (!existing) {
        // Insert with explicit item_id to converge IDs across devices
        const normalized = normalizeItemForDb(item);
        db.prepare(`
          INSERT INTO items(
...
export function importSnapshotLww(db, snapshot) {
  const tx = db.transaction(() => {
    ...
    // locations (best-effort; parent links may need multiple passes)
    ...
    // items - trust item_id to converge across devices
    if (Array.isArray(snapshot.items)) {
      upsertManyByIdLww(db, snapshot.items);
    }

    // item_barcodes (best-effort). If a barcode is already mapped differently locally, keep local mapping.
